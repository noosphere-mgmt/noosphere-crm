...
const FIELD_KEYS = [
  "lead_id", "status", "contact_name", "company_name", "email", "phone", "website", "source",
  "email_subject", "email_excerpt", "email_message_id", "email_thread_id", "requirement_notes", "ai_digest",
  "office_space_required", "next_lease_expiry", "required_area_sqft", "required_capacity_pax", "preferred_location",
  "assigned_owner", "virtual_staff", "qualification_score", "qualification_reason", "last_email_at",
  "next_follow_up_date", "converted_company_id", "converted_contact_id", "converted_opportunity_id", "converted_at",
] as const;

const SELECT = FIELD_KEYS.map((key) => key === "lead_id" ? "id::text AS lead_id" : key).join(", ");

function fieldDef(key: (typeof FIELD_KEYS)[number]): ImportFieldDef {
  const base = { key, label: key };
  if (key === "lead_id") return { ...base, type: "number", integer: true, matchOnly: true, aliases: ["id"] };
  if (["office_space_required"].includes(key)) return { ...base, type: "boolean" };
  if (["required_area_sqft", "required_capacity_pax", "qualification_score"].includes(key)) return { ...base, type: "number", integer: key !== "required_area_sqft" };
  if (["next_lease_expiry", "next_follow_up_date"].includes(key)) return { ...base, type: "date" };
  if (key === "status") return { ...base, type: "enum", enumValues: ["new", "reviewing", "qualified", "converted", "nurture", "disqualified", "duplicate"], defaultValue: "new" };
  if (["converted_company_id", "converted_contact_id", "converted_opportunity_id"].includes(key)) return { ...base, type: "number", integer: true };
...
export const leadsImportDefinition: ImportObjectDefinition = {
  ...
  async findById(id) { return (await load("id = $1", [Number(id)]))[0] ?? null; },
  async findByExternalRef(externalRef) { return load("email_message_id = $1", [externalRef.trim()]); },
  ...
  async exportRows() {
    const rows = await query<Record<string, unknown>>(`SELECT ${SELECT} FROM leads ORDER BY updated_at DESC`);
