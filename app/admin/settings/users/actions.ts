...
export async function createCrmUserAction(f:FormData){const id=await createCrmUser(data(f));revalidatePath("/admin/settings/users");redirect(`/admin/settings/users/${id}?saved=1`);}
export async function updateCrmUserAction(id:number,f:FormData){await updateCrmUser(id,data(f));revalidatePath("/admin/settings/users");revalidatePath(`/admin/settings/users/${id}`);redirect(`/admin/settings/users/${id}?saved=1`);}
