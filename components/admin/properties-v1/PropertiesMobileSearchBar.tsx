...
export function PropertiesMobileSearchBar({ initialQuery, initialCategory, initialTitle, initialRelatedCompany }: { initialQuery?: string; initialCategory?: string; initialTitle?: string; initialRelatedCompany?: string }) {
  ...
  useEffect(() => setRelatedCompany(initialRelatedCompany ?? ""), [initialRelatedCompany]);
  ...
  return (
    <div className={`mb-2 grid grid-cols-2 gap-2 ${isPending ? "opacity-70" : ""}`}>
      ...
      />
      <select aria-label="Building Type" value={initialCategory ?? ""} onChange={(event) => setFilter("category", event.target.value)} className={theme.searchSelect}>
      ...
      <input type="search" value={relatedCompany} onChange={(event) => setRelatedCompany(event.target.value)} placeholder="Related company" aria-label="Related company" className={`${theme.searchInput} w-full py-1.5 text-sm`} />
    </div>
  );
}
