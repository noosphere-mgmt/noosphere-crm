...
export function PremisesInlineOverviewMobile({
  premises,
  buildingName,
  propertyOptions,
  companies,
  ...
  return (
    <div className="space-y-3">
      ...
      <PremisesSectionCard title="Key snapshot">
        {buildingName?.trim() ? (
          <p className="mb-1 text-sm font-semibold text-slate-900">{buildingName.trim()}</p>
        ) : null}
        <p className="mb-3 text-base font-bold text-blue-800">{premiseLabel}</p>
      ...
      {forLease ? (
        <PremisesSectionCard title="Pricing / lease terms">
          <div className="grid grid-cols-2 gap-3">
            ...
            <InlineTextField label="Deposit (months)" value={premises.deposit_months} onSave={save("deposit_months")} />
            <InlineTextField
              label="Rent-free period"
              value={premises.rent_free_period}
              onSave={save("rent_free_period")}
            />
            <InlineTextField
              label="Contract term (months)"
              value={premises.contract_term_months?.toString() ?? null}
              type="number"
              onSave={save("contract_term_months")}
            />
            <InlineTextField
              label="Available date"
              value={premises.available_date?.slice(0, 10) ?? null}
              type="date"
              onSave={save("available_date")}
            />
          </div>
          ...
          <p className="mt-2 text-xs text-slate-500">
            ...
            {formatMoney(premises.monthly_rent, currency)} · {formatPsf(premises.rent_psf, currency)} PSF
          </p>
        </PremisesSectionCard>
      ) : null}
